...
{
    //#ifdef DEBUG_PRINTS
    //std::cout<<"** [MC-VEO_TASK EVENTS] Received "<<events_sample.events.size()<<" events at["<<events_sample.time.toSeconds()<<"] **\n";
    //#endif
    ...
    if (this->events.size() >= this->mc_veo_config.data_loader.num_events)
    {
        ...
        std::chrono::duration<double, std::milli> cf_ms = t2 - t1;
        std::cout<<"** [MC-VEO_TASK] CREATE EVENT IMAGE:"<< cf_ms.count()<<" ms**\n";

        /** Increment EF INDEX **/
        this->ef_idx++;

        /** MC-VEO TRACKER OPTIMIZATION **/
        if (this->initialized)
        {
            ...
            #ifdef DEBUG_PRINTS
            std::cout<<"** [MC-VEO_TASK EVENTS] Wrote pose_kf_ef:\n"<<this->pose_kf_ef.getTransform().matrix()<<std::endl;
            #endif
            ...
            #ifdef DEBUG_PRINTS
            std::cout<<"** [MC-VEO_TASK EVENTS] Wrote pose_w_ef:\n"<<this->pose_w_ef.getTransform().matrix()<<std::endl;
            #endif
            ...
            std::cout<<"** [MC-VEO_TASK] DEAL EVENT:"<< fp_ms.count()<<" ms**\n";
            this->ev_time += fp_ms.count();
            this->op_ev_num += 1;
            std::cout << "[MC-VEO_TASK] ev_time ["<<this->ev_time<<"]\n";
            std::cout << "[MC-VEO_TASK] op_ev_num ["<<this->op_ev_num<<"]\n";
            std::cout << "[MC-VEO_TASK] ev_time/op_ev_num ["<<(this->ev_time/this->op_ev_num)<<"]\n";
        }
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> al_ms = end - t1;
        std::cout<<"** [MC-VEO_TASK] ALL EVENT:"<< al_ms.count()<<" ms**\n";
        this->al_ev_time += (long long)al_ms.count();
        std::cout << "[MC-VEO_TASK] al_ev_time ["<<this->al_ev_time<<"]\n";
    }
...
{
    #ifdef DEBUG_PRINTS
    std::cout<<"** [MC-VEO_TASK] FRAME IDX:"<< this->frame_idx<<" Received Frame at ["<<frame_sample->time.toSeconds()<<"]**\n";
    #endif

    /** Get the image in DSO format **/
    dso::ImageAndExposure* img = this->getImageAndExposure(*frame_sample);
    std::cout<<"[MC-VEO_TASK] img time: "<<img->timestamp<<" size: "<<img->w<<"x"<<img->h <<std::endl;

    /** Get current info **/
    size_t window_size = this->frame_hessians.size();
    std::cout<<"[MC-VEO_TASK FRAME] Keyframes size (Hessian frames): "<< window_size<<" All Frames size: "<<this->all_frame_history.size()
    ...
    if (this->initialized)
    {
        ...
        if (this->kf_idx < this->frame_hessians[this->frame_hessians.size()-1]->frameID)
        {
            state(LOGGING);
            {
                ...
                {
                    std::cout<<"[MC-VEO_TASK] KF ID: "<<it->frameID<<" FRAME ID: "<<it->shell->id<<" NUM POINTS: "<<it->pointHessians.size()
       ...
       std::cout<<"[MC-VEO_TASK] FRAME HISTORY SIZE: "<<this->all_frame_history.size()<<" BUNDLES MAP POINTS: "<<this->bundles->nPoints<<std::endl;
    }
    else
    {
        state(INITIALIZING);
        ...
        if (first_frame)
            std::cout<<"[MC-VEO_TASK] INIT FIRST FRAME ID: "<<first_frame->shell->incoming_id;
        if (new_frame)
            std::cout<<" NEW FRAME ID: "<<new_frame->shell->incoming_id;
        std::cout<<"\n";
        std::cout<<"[MC-VEO_TASK] INIT T_INIT:\n"<<this->initializer->thisToNext.matrix3x4()<<std::endl;
        std::cout<<"[MC-VEO_TASK] INIT FRAME ID: "<<this->all_frame_history[this->frame_idx]->id <<"\n T_world_cam:\n"
...
bool Task::configureHook()
{
    ...
    std::cout<<"** [MC-VEO_TASK CONFIG] Configuration CAM1: **"<<std::endl;
    ...
    std::cout<<"** [MC-VEO_TASK CONFIG] Frame intrinsics [";print(this->cam_calib.cam0.intrinsics); std::cout<<"]"<<std::endl;
    std::cout<<"** [MC-VEO_TASK CONFIG] Events intrinsics [";print(this->cam_calib.cam1.intrinsics); std::cout<<"]"<<std::endl;
    ...
    std::cout<<"** [MC-VEO CONFIG] fxl: "<<this->calib->fxl()<<" fxli: "<<this->calib->fxli()<<std::endl;
    std::cout<<"** [MC-VEO CONFIG] fyl: "<<this->calib->fyl()<<" fyli: "<<this->calib->fyli()<<std::endl;
    std::cout<<"** [MC-VEO CONFIG] cxl: "<<this->calib->cxl()<<" cxli: "<<this->calib->cxli()<<std::endl;
...
::mc_veo::DataLoaderConfig Task::readDataLoaderConfig(YAML::Node config)
{
    ...
    dt_config.motion_compensation = config["motion_compensation"].as<bool>();
    if (dt_config.motion_compensation)
    {
        dt_config.overlap = config["new_overlap"].as<double>(); //in percentage
    }
    else
    {
        dt_config.overlap = config["overlap"].as<double>(); //in percentage
...
dso::SE3 Task::initialize(dso::ImageAndExposure* image, int id, const int &snapped_threshold)
{
    ...
    else if(this->initializer->trackFrame(fh, snapped_threshold))	// if SNAPPED
    {
        ...
        {
            std::cout<<"[MC-VEO_TASK] START INITIALIZE FROM DSO INITIALIZER"<<std::endl;

            /** add first keyframe: this is what MC-VEO calls add frame to Hessian struct **/
            ...
            if(!dso::setting_debugout_runquiet)
                printf("[MC-VEO_TASK] Initialization: keep %.1f%% (need %d, have %d)!\n", 100*keepPercentage,
            ...
            /** Optimize the first two DSO Keyframes and set the first MC-VEO Keyframe**/
            ...
            std::cout<<"** [MC-VEO_TASK] INITIALIZE FROM INITIALIZER ("<< (int)firstFrame->pointHessians.size() <<" pts) "<< rescaleFactor <<" rescale_factor!"<<std::endl;
            std::cout<<"** [MC-VEO_TASK] INITIALIZATION FINISHED WITH T_w_kf:\n"<< this->pose_w_kf.getTransform().matrix()<<std::endl;
        }
    }
    else
    {
...
bool Task::eventsToImageAlignment(const std::vector<::base::samples::Event> &events_array, ::base::Transform3d &T_kf_ef)
{
    ...
    #ifdef DEBUG_PRINTS
    std::cout<<"** [MC-VEO_TASK EVENT_TO_IMG ALIGNMENT] "<<(success?"SUCCESS": "NO_USABLE") <<" rescale_factor: "<< this->rescale_factor<<"\nT_kf_ef:\n"<<T_kf_ef.matrix()<<std::endl;
    #endif
    state(RUNNING);
...
bool Task::eventsToImageAlignment(const Eigen::Affine3d &pose_w_kf,const std::vector<::base::samples::Event> &events_array, ::base::Transform3d &T_kf_ef)
{
    /** Keyframe to Eventframe delta pose **/
    T_kf_ef = ::base::Transform3d::Identity();
...
void Task::track(dso::ImageAndExposure* image, int id)
{
    ...
    std::cout<<"** [MC-VEO_TASK] "<<(this->event_tracker->needNewKeyframe() ?"NEW KEYFRAME": "NO KEYFRAME")<<std::endl;
    std::cout<<"[MC-VEO_TASK] tres[3]: "<<tres[3]<<" sqrtf(tres[3]/(640*480): "<<sqrtf((double)tres[3]) / (dso::wG[0]+dso::hG[0])
    ...
    if (!create_kf) std::cout<<"[MC-VEO_TRACK]: NO NEED NEW KF"<<std::endl; else std::cout<<"[MC-VEO_TRACK]: CREATE NEW KF"<<std::endl;
...
void Task::printResult(std::string file)
{
	...
	for(dso::FrameShell* s : this->all_frame_history)
	{
		...
		myfile << s->timestamp <<
			" " << s->camToWorld.translation().transpose()[0]<<
			" " << s->camToWorld.translation().transpose()[1]<<
			" " << s->camToWorld.translation().transpose()[2]<<
